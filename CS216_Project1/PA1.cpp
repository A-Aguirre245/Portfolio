            ...
            while (getline(iss, movieTitle, ';'))
{
    ...
    while (true)
    {
	    ...
	    cin.clear();
            ...
            cin >> input_to_check;
            if (input_to_check == "Q" || input_to_check == "q" || cin.eof())
                break;
            else
            {
...
void analyzeMovies(const IMDB& imdb)
{
	...
	//this uses the union and intersection to help find the difference
...
void analyzeCoActors(const IMDB& imdb)
{
    ...
    for (set<string>::iterator im = movies_of_actor.begin(); im != movies_of_actor.end(); im++)
    {
        ...
        set<string> coactors;
	coactors = imdb.find_actors_in_a_movie(*im);
